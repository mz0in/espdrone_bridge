...
#ifndef INCLUDE_aerial_catcher_COMMON_H_
#define INCLUDE_aerial_catcher_COMMON_H_
...
#include "aerial_catcher/parameters.h"

namespace aerial_catcher
{
  ...
  inline void eigenOdometryFromMsgOdom(nav_msgs::Odometry msg,
                                       EigenOdometry *odometry)
  {
    odometry->position = mav_msgs::vector3FromPointMsg(msg.pose.pose.position);
    odometry->orientation = mav_msgs::quaternionFromMsg(msg.pose.pose.orientation);
  ...
  inline double line_inter(double a, double b, double c)
  {
    assert(a < b);
    assert(a < c);
    assert(b < c);
  ...
  inline double line_inter3(Eigen::Vector3d a, Eigen::Vector3d b, Eigen::Vector3d c)
  {
    double dist_ab, dist_ac, dist_bc;
    dist_ab = sqrt(pow(a[0] - b[0], 2) + pow(a[1] - b[1], 2) + pow(a[2] - b[2], 2));
    dist_ac = sqrt(pow(a[0] - c[0], 2) + pow(a[1] - c[1], 2) + pow(a[2] - c[2], 2));
    return dist_ac / dist_ab;
  }

  inline double line_inter3_d(Eigen::Vector3d a, Eigen::Vector3d b, Eigen::Vector3d c)
  {
    double dist_ab, dist_ac, dist_bc;
    dist_ab = sqrt(pow(a[0] - b[0], 2) + pow(a[1] - b[1], 2) + pow(a[2] - b[2], 2));
    dist_ac = sqrt(pow(a[0] - c[0], 2) + pow(a[1] - c[1], 2) + pow(a[2] - c[2], 2));
    dist_bc = sqrt(pow(b[0] - c[0], 2) + pow(b[1] - c[1], 2) + pow(b[2] - c[2], 2));
    return 1 - dist_bc / (dist_ac + dist_bc);
  }
}

#endif /* INCLUDE_aerial_catcher_COMMON_H_ */
